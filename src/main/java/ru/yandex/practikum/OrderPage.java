...
public class OrderPage extends BasePage {
    private By orderNameField = By.xpath("//input[@placeholder='* Имя']"); //локатор поля ввода "Имя" на странице заказа
    private By orderLastNameField = By.xpath("//input[@placeholder='* Фамилия']"); //локатор поля ввода "Фамилия" на странице заказа
    private By orderAddressField = By.xpath("//input[@placeholder='* Адрес: куда привезти заказ']"); //локатор поля "Адрес: куда привезти заказ" на странице заказа
    private By orderMetroStationField = By.className("select-search__input"); //локатор поля ввода "Станция метро" на странице заказа
    private By orderMetroStationFieldFull = By.xpath("//*[@class='select-search__select']"); // локатор выпадающего списка метро на странице заказа
    private By orderNumberPhoneField = By.xpath("//input[@placeholder='* Телефон: на него позвонит курьер']"); //локатор поля ввода "Телефон: на него позвонит курьер" на странице заказа
    ...
    private By checkBoxBlackColorScooter = By.xpath("//input[@id='black']"); // локатор чекбокса "Черный жемчуг" на странице заказа
    private By orderButtonOnOrderPage = By.xpath("//button[@class='Button_Button__ra12g Button_Middle__1CSJM']"); // локатор кнопки "Заказать" на странице заказа
    private By orderConfirmationButton = By.xpath("//button[text()='Да']"); // локатор кнопки "Да" на странице подтверждения заказа
